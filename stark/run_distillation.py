...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Type of teacher model, tuning or prompt",   
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Data augmentation percent.",
    )
    parser.add_argument(
        "--output_dir", 
        type=str, 
        default="outputs/distill", 
    ...
    parser.add_argument("--do_distill", action="store_true", help="Do distill or not.")
    parser.add_argument("--do_test", action="store_true", help="Do test or not.")
    parser.add_argument("--student_layers", type=int, default=4, help="Suffix for outputs.")
...
def main():
    args = parse_args()
    if args.data_aug:
        args.output_dir = os.path.join(args.output_dir, f"{args.task_name}_{args.teacher_type}_" + "aug")
    else:
        args.output_dir = os.path.join(args.output_dir, f"{args.task_name}_{args.teacher_type}")
    ...
    # Disitll is conducted in certain accelaration.
    if args.do_distill:
        ...
        if args.data_aug:
            train_examples = data_reader.get_aug_examples()
        else:
        ...
        if args.student_sparsity == "":
            add_kwargs_to_config(
                s_config,
                num_labels=num_labels,
                num_hidden_layers=args.student_layers, # 4/12~=0.3, 0.7 sparsity
            )
        ...
        logger.info(f"***** Running distillation *****")
        ...
        logger.info(f"***** Finalizing distillation *****")
    ...
    if args.do_test and is_main:
        ...
        if is_dist:
            test_sampler = DistributedSampler(test_instances, shuffle=True)
        else:
           test_sampler = RandomSampler(test_instances)
